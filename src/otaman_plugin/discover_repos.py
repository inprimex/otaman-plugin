...
def scan_directory(root: Path) -> dict[str, Any]:
    ...
    for i, entry in enumerate(entries, 1):
        ...
        # Skip maestro/otaman folders. Three signals:  # legacy: pre-rebrand reference
        ...
        is_otaman_named = entry.name.endswith("-otaman") or entry.name.endswith(
            "-maestro"  # legacy: pre-rebrand reference
        )
...
def generate_draft_yaml(
    ...
    for repo in report["repos"]:
        # Compute path relative to maestro dir (not scan root)  # legacy: pre-rebrand reference
        if maestro_dir:
    ...
    if report.get("openspec"):
        ...
        # When maestro hosts openspec, path is local  # legacy: pre-rebrand reference
        if maestro_dir:
            config["specs"] = {
                "path": "./openspec",
                "format": "fallback",
...
def main() -> int:
    ...
    # Parse --maestro-dir option  # legacy: pre-rebrand reference
    ...
    for i, arg in enumerate(sys.argv[1:], 1):
        if skip_next:
            skip_next = False
            continue
        if arg == "--maestro-dir" and i < len(sys.argv) - 1:  # legacy: pre-rebrand reference
    ...
    # Create maestro dir if specified and doesn't exist  # legacy: pre-rebrand reference
    if maestro_dir:
        if not dry_run:
