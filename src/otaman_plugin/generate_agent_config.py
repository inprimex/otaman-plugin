...
def _render_git_policy_section(
    ...
    ``policy/`` — the two are independent, not sequenced.

    CAVEAT actually load-bearing: narrow-only LOOSENING PROTECTION (a repo/
    agent override may not weaken force_push_forbidden/
    require_status_checks/etc.) only applies once ``policy/index.yaml``
    registers the pack's narrow-only set — without an index on disk, a
    repo/agent override CAN silently loosen a narrow-only rule with no
    :class:`LooseningViolation` recorded. This is why ``main()`` calls
    ``install_policy_files`` BEFORE this function runs for any repo: by the
    time this renders, the index this same generation pass just wrote is
    already in place, so the composition algebra's protection is live from
    the very first ``otaman init``, not just the second.
    """
...
_CI_GATE_WORKFLOW_NAME = "otaman-ci-gate.yml"

_CI_GATE_TEMPLATE = """\
# Generated by otaman-plugin (policy-engine 2.2). NOT overwritten on
# subsequent `init --update` runs once this file exists — edit freely.
#
# A canonical, stably-named aggregation check ("ci-ok") that fans in this
# repo's own existing jobs. Point branch protection's required status check
# at "ci-ok" instead of a job-specific name that varies per repo (the
# otaman-dev fleet had drifted to a mix of "ci-ok" and "lint-and-test" —
# policy-engine design D4a) — the deploy-applied protection setting is
# resolved separately, with a live read of this repo's actual check runs.
#
# Job names below were discovered by reading this repo's OTHER
# .github/workflows/*.yml files at generation time. If they've since
# changed, update the `needs:` list to match — this file is yours now.
name: otaman-ci-gate
...
def _discover_repo_ci_job_names(repo_dir: Path) -> list[str]:
    """Read job names from ``repo_dir/.github/workflows/*.yml`` — a local
    filesystem read, same as every other generator function; no network
    I/O and no dependency on a live GitHub API token.

    This is deliberately NOT the same read as policy-engine D4a's
    "required-check context read from the repo's live CI" — that's the
    context string GitHub branch protection enforces, resolved by deploy
    (step 3) with a live check-runs API read at protection-apply time.
    This is only about what to list in the generated gate's own
    ``needs:`` so it fans in the repo's real jobs.
    """
    ...
    for wf_file in sorted(workflows_dir.glob("*.yml")) + sorted(workflows_dir.glob("*.yaml")):
        if wf_file.name == _CI_GATE_WORKFLOW_NAME:
            continue
        try:
            doc = _yaml.safe_load(wf_file.read_text(encoding="utf-8"))
        except Exception:
            continue
        if isinstance(doc, dict) and isinstance(doc.get("jobs"), dict):
            jobs.update(doc["jobs"].keys())
    return sorted(jobs)


def install_ci_gate_templates(project_root: Path, config: dict[str, Any]) -> list[str]:
    """policy-engine 2.2: materialize a canonical ``ci-ok`` aggregation-gate
    workflow for each repo whose EFFECTIVE git policy requires status
    checks (``require_status_checks``), when the repo doesn't already have
    one. Never overwrites an existing gate file — a repo that already
    defines its own gate keeps it untouched (D1 generate-and-diff).

    Skipped (no-op, not a warning) for a repo with no discoverable jobs yet
    — nothing to fan in. Guarded: an older/absent ``otaman_core.policy``
    ...
    for repo in config.get("repos", []):
        try:
            ...
            if not repo_dir.is_dir():
                continue
            gate_path = repo_dir / ".github" / "workflows" / _CI_GATE_WORKFLOW_NAME
            if gate_path.exists():
                continue
            ...
            jobs = _discover_repo_ci_job_names(repo_dir)
            if not jobs:
                continue  # nothing to gate on yet

            content = _CI_GATE_TEMPLATE.format(
                default_branch=repo.get("default_branch", "main"),
                needs_list=json.dumps(jobs),
            )
            gate_path.parent.mkdir(parents=True, exist_ok=True)
            gate_path.write_text(content, encoding="utf-8")
            results.append(f"Created: {repo['name']}/.github/workflows/{_CI_GATE_WORKFLOW_NAME}")
        except Exception:
            continue
    return results
...
def main() -> int:
    ...
    # policy below. otaman_core.policy's narrow-only loosening protection
    # only applies once policy/index.yaml registers a pack's narrow-only
    # rules — on a fresh program (no policy/ on disk yet), a repo/agent
    # override that attempts to loosen a narrow-only rule (e.g.
    # require_status_checks) is NOT refused until the index exists. Running
    # this first closes that window so the very first generation pass
    # already reflects the protected (tightest-wins) value everywhere else
    # that calls effective_policy (CLAUDE.local.md rule text, ci-gate
    # templates) — never a transiently-loosened one.
    ...
    # Materialize missing per-repo ci-ok gate workflows (never overwrites)
