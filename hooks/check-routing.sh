#!/usr/bin/env bash
# SessionStart: warn when the active otaman routing disagrees with the
# .otaman marker's expected_routing (or legacy expected_account). Never blocks — only writes to stderr.
#
# Resolution order (mirrors bridge_approval.py's _derive_account):
#   1. ${OTAMAN_ACTIVE_ROUTING:-${OTAMAN_ACTIVE_ACCOUNT:-${MAESTRO_ACTIVE_ACCOUNT:-}}}  — set by the launcher, most reliable
...
#
# OTAMAN_ACTIVE_ROUTING takes priority because it's how projects that
# share a single CLAUDE_CONFIG_DIR (one login per subscription, many
# Telegram groups) tell the hook which routing is *actually* active.
set -u
...
expected="$(read_expected_routing "$PWD")" || exit 0
[[ -n "$expected" ]] || exit 0

actual=""
source_desc=""

if [[ -n "${OTAMAN_ACTIVE_ROUTING:-${OTAMAN_ACTIVE_ACCOUNT:-${MAESTRO_ACTIVE_ACCOUNT:-}}}" ]]; then
    actual="${OTAMAN_ACTIVE_ROUTING:-${OTAMAN_ACTIVE_ACCOUNT:-${MAESTRO_ACTIVE_ACCOUNT:-}}}"
    source_desc="OTAMAN_ACTIVE_ROUTING=$actual"
