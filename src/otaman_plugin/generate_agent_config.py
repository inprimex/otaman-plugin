...
def _render_spec_lifecycle_note(config: dict[str, Any]) -> str:
    ...
    which resolves ``(None, program_block)`` the same way).

    Degrades to "" — never blocks generation — when an older core lacks
    the module or resolution raises (same optional-core-feature
    convention as this file's other ``_render_*`` sections).
    """
    try:
        from otaman_core.spec_lifecycle import STAGES, resolve_spec_policy

        policy = resolve_spec_policy(None, config.get("spec_policy"))
        stages = " → ".join(STAGES)
        mode_note = (
            "a violation refuses the operation outright"
            if policy.enforcement == "block"
            else "a violation proceeds but is never silent — it prints a notice"
        )
        return f"""

### Spec lifecycle (stage machine + gates)
Every spec change moves through: `{stages}`. Three LOCAL gates enforce
this (no CI or git-host integration required) — **merge** (needs
`approved_by`), **dispatch** (needs `spec-approved`), **archive** (needs
`approved_by`); research-stage changes are exempt from all three. This
program's enforcement mode is **{policy.enforcement}** — {mode_note}.
`self-waive`, where configured, is ALWAYS visible in the gate's notice
...
def _build_maestro_block(
    ...
    spec_lifecycle_note = _render_spec_lifecycle_note(config)
...
_GIT_REMOTE_ORG_RE = re.compile(
    r"^(?:git@[^:]+:|https?://[^/]+/|ssh://[^/]+/)?([^/]+)/[^/]+?(?:\.git)?$"
)


def _github_org_from_remote(remote: str) -> str | None:
    """Extract the GitHub org/owner segment from a repo's `remote:` URL —
    e.g. ``git@github.com:inprimex/otaman-core.git`` -> ``inprimex``.

    This is deliberately the GitHub ORG, not platform.yaml's per-repo
    `owner:` field (an internal agent/human identity, not a GitHub
    account) — there is no distinct GitHub account per agent; every repo
    in this fleet is pushed under the same org/account. Matches the one
    real precedent in this org (otaman-core/.github/CODEOWNERS: `* @inprimex`).
    """
    match = _GIT_REMOTE_ORG_RE.match(remote.strip())
    return match.group(1) if match else None


def install_codeowners_files(project_root: Path, config: dict[str, Any]) -> list[str]:
    """policy-engine 2.2: materialize a minimal `.github/CODEOWNERS` for each
    repo whose EFFECTIVE git policy requires owner admission
    (``owner_admission_required`` / ``agents_merge_human_owned_branch_forbidden``),
    default-branch owner only (`* @org`) — never overwrites an existing file
    (D1 generate-and-diff; a repo's own CODEOWNERS, once present, is canon).

    Deliberately does NOT attempt to express per-branch/per-agent owner
    identity here — GitHub CODEOWNERS gates by file path, not by branch,
    so ad-hoc-branch ownership stays the cli merge guard's job (2.1). See
    the amended policy-pack-git spec delta (spec-agent design check,
    2026-09-01): CODEOWNERS = repo-level default-branch owner only.

    Skipped (no-op) for a repo with no `remote:` (nothing to resolve an
    org from) or whose effective policy doesn't require it. Guarded: an
    older/absent ``otaman_core.policy`` degrades to a no-op, same
    convention as this file's other optional-core-feature installers.
    """
    results: list[str] = []
    try:
        from otaman_core.policy import effective_policy
    ...
    for repo in config.get("repos", []):
        try:
            ...
            if not remote:
                continue
            org = _github_org_from_remote(remote)
            if not org:
                continue
            ...
            if not repo_dir.is_dir():
                continue
            codeowners_path = repo_dir / ".github" / "CODEOWNERS"
            ...
            rules = effective.rules
            if not (
                rules.get("owner_admission_required")
                or rules.get("agents_merge_human_owned_branch_forbidden")
            ):
                continue

            content = (
                f"# Generated by otaman-plugin (policy-engine 2.2). NOT overwritten on\n"
                f"# subsequent `init --update` runs once this file exists — edit freely.\n"
                f"#\n"
                f"# Default owner for everything in the repo — repo-level, default-branch\n"
                f"# only. Per-branch/per-agent owner identity is enforced separately by\n"
                f"# `otaman policy check-merge` (policy-engine 2.1), not expressible here.\n"
                f"*       @{org}\n"
            )
            codeowners_path.parent.mkdir(parents=True, exist_ok=True)
            codeowners_path.write_text(content, encoding="utf-8")
            results.append(f"Created: {repo['name']}/.github/CODEOWNERS")
        except Exception:
            continue
    return results
...
_SPEC_LIFECYCLE_GATE_WORKFLOW_NAME = "otaman-spec-lifecycle-gate.yml"

_SPEC_LIFECYCLE_GATE_TEMPLATE = """\
# Generated by otaman-plugin (spec-lifecycle-enforcement 3.1). NOT
# overwritten on subsequent `init --update` runs once this file exists —
# edit freely.
#
# Validates stage/approved_by on every changed change's .openspec.yaml on
# PR — the MERGE gate (design D2), enforced via otaman-core's pure
# check_merge_gate. Research-stage changes are exempt (handled inside
# check_merge_gate itself); a legacy/unbackfilled change with no
# .openspec.yaml is skipped, not failed (D1: repo is truth, absence is
# not yet an opinion). This is the STRONGEST layer where GitHub
# integration exists — a CI-less tenant gets the same gate locally via
# `otaman spec gate <change> --at merge`; this workflow is additive,
# never the only enforcement (D2).
#
# The enforcement mode below is baked in at generation time from this
# program's platform.yaml `spec_policy:` block (program layer only) —
# re-run `otaman init --update` after changing it to regenerate. Only
# otaman-core is checked out (sibling) since check_merge_gate is a pure
# function; no otaman-cli or platform.yaml access needed at CI runtime.
name: otaman-spec-lifecycle-gate
...
jobs:
  spec-lifecycle-gate:
    ...
    steps:
      ...
      - name: Checkout otaman-core (sibling — pure gate-check logic)
        uses: actions/checkout@v7
        with:
          repository: {core_org}/otaman-core
          path: _otaman-core-sibling
      ...
      - name: Install otaman-core
        run: pip install ./_otaman-core-sibling

      - name: Find changes touched by this PR
        id: changes
        run: |
          git fetch origin "${{{{ github.event.pull_request.base.ref }}}}" --depth=1
          touched=$(git diff --name-only "origin/${{{{ github.event.pull_request.base.ref }}}}...HEAD" -- openspec/changes/ \\
            | sed -E 's#^openspec/changes/([^/]+)/.*#\\1#' | sort -u)
          {{
            echo "touched<<SPEC_GATE_EOF"
            echo "$touched"
            echo "SPEC_GATE_EOF"
          }} >> "$GITHUB_OUTPUT"

      - name: Run the merge gate on each touched change
        env:
          ENFORCEMENT_MODE: "{enforcement}"
          TOUCHED: ${{{{ steps.changes.outputs.touched }}}}
        run: |
          python3 - <<'SPEC_GATE_PYEOF'
          import os
          import sys

          import yaml
          from otaman_core.spec_lifecycle import SpecPolicy, check_merge_gate

          policy = SpecPolicy(enforcement=os.environ["ENFORCEMENT_MODE"])
          touched = [c for c in os.environ.get("TOUCHED", "").splitlines() if c]
          failed = False
          for name in touched:
              path = f"openspec/changes/{{name}}/.openspec.yaml"
              if not os.path.isfile(path):
                  continue  # legacy/unbackfilled change -- not gated until it has a stage
              with open(path, encoding="utf-8") as f:
                  data = yaml.safe_load(f) or {{}}
              decision = check_merge_gate(data, policy)
              for notice in decision.notices:
                  print(f"::warning file={{path}}::{{notice}}")
              if not decision.allowed:
                  for violation in decision.violations:
                      print(f"::error file={{path}}::{{violation}}")
                  failed = True
...
def install_spec_lifecycle_ci_gate(project_root: Path, config: dict[str, Any]) -> list[str]:
    """spec-lifecycle-enforcement 3.1: materialize the spec-repo-only CI
    gate workflow validating ``stage``/``approved_by`` on PR (the MERGE
    gate, D2). Never overwrites an existing file (D1/D5-style widen-by-
    delta convention: a repo's own CI, once present, is canon) — matches
    this file's other CI-template installers.

    Applies ONLY to the specs repo (matched the same way
    ``install_repo_post_commit_hooks`` skips it: ``repo["path"]`` equal
    to ``config["specs"]["path"]``) — this gate validates
    ``openspec/changes/*/.openspec.yaml``, which only exists there.
    Skipped (no-op) when ``specs`` is absent from config, the specs repo
    isn't checked out locally, or ``otaman-core`` isn't a registered repo
    with a resolvable GitHub org (needed for the sibling-checkout step —
    the same derivation ``install_codeowners_files`` uses).
    """
    results: list[str] = []
    specs_cfg = config.get("specs")
    if not isinstance(specs_cfg, dict):
        return results
    specs_path = specs_cfg.get("path", "")
    if not specs_path:
        return results

    core_repo = next((r for r in config.get("repos", []) if r.get("name") == "otaman-core"), None)
    core_remote = core_repo.get("remote") if core_repo else None
    core_org = _github_org_from_remote(core_remote) if core_remote else None
    if not core_org:
        return results
    ...
    for repo in config.get("repos", []):
        try:
            ...
            if not repo_path_str or repo_path_str.rstrip("/") != specs_path.rstrip("/"):
                continue

            repo_dir = (project_root / repo_path_str).resolve()
            if not repo_dir.is_dir():
                continue

            gate_path = repo_dir / ".github" / "workflows" / _SPEC_LIFECYCLE_GATE_WORKFLOW_NAME
            if gate_path.exists():
                continue
            ...
            policy = resolve_spec_policy(None, config.get("spec_policy"))
            content = _SPEC_LIFECYCLE_GATE_TEMPLATE.format(
                core_org=core_org, enforcement=policy.enforcement
            )
            gate_path.parent.mkdir(parents=True, exist_ok=True)
            gate_path.write_text(content, encoding="utf-8")
            results.append(
                f"Created: {repo['name']}/.github/workflows/{_SPEC_LIFECYCLE_GATE_WORKFLOW_NAME}"
            )
        except Exception:
            continue
    return results
...
def main() -> int:
    ...
    # Materialize the specs-repo-only spec-lifecycle CI gate (merge gate,
    # stage/approved_by validation) when absent
    spec_lifecycle_gate_results = install_spec_lifecycle_ci_gate(project_root, config)
    for r in spec_lifecycle_gate_results:
        print(r)
