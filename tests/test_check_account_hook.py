"""Tests for hooks/check-account.sh — account mismatch warning.

Verifies the resolution order that mirrors bridge_approval.py's
_derive_account (OTAMAN_ACTIVE_ACCOUNT → CLAUDE_CONFIG_DIR basename
...
HOOK = REPO_ROOT / "hooks" / "check-account.sh"
...
def _write_marker(workspace, expected_account: str) -> None:
...
def _run(workspace, *, env_extra=None, unset=None):
    env = os.environ.copy()
    for k in ("OTAMAN_ACTIVE_ACCOUNT", "CLAUDE_CONFIG_DIR"):
        env.pop(k, None)
    if env_extra:
...
class TestActiveAccountEnvVarWins:
    """OTAMAN_ACTIVE_ACCOUNT is the first signal — it disambiguates the
    ...
    def test_env_matches_marker_no_warning(self, workspace):
        ...
        result = _run(workspace, env_extra={
            "OTAMAN_ACTIVE_ACCOUNT": "greenbin",
    ...
    def test_env_overrides_config_dir_basename(self, workspace):
        ...
        CLAUDE_CONFIG_DIR says 'personal' but OTAMAN_ACTIVE_ACCOUNT says
    ...
    def test_env_mismatch_warns(self, workspace):
        _write_marker(workspace, "greenbin")
        result = _run(workspace, env_extra={
            "OTAMAN_ACTIVE_ACCOUNT": "someone-else",
        })
        assert result.returncode == 0
        assert "mismatch" in result.stderr
        assert "someone-else" in result.stderr
        assert "greenbin" in result.stderr
        assert "OTAMAN_ACTIVE_ACCOUNT" in result.stderr


class TestConfigDirFallback:
    """When OTAMAN_ACTIVE_ACCOUNT isn't set, fall back to
    ...
    def test_mismatched_basename_warns(self, workspace):
        ...
        assert "OTAMAN_ACTIVE_ACCOUNT" not in result.stderr
...
class TestNoMarker:
    def test_no_marker_silent_skip(self, workspace):
        ...
        result = _run(workspace, env_extra={
            "OTAMAN_ACTIVE_ACCOUNT": "anything",
        })
        assert result.returncode == 0
